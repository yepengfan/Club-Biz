class Society < ActiveRecord::Base
	...
	def update(society_params)
		if society_params[:desc]
			self.desc = society_params[:desc]
		end
		if society_params[:website]
			self.website = society_params[:website]
		end

		if society_params[:p_first_name]
			self.p_first_name = society_params[:p_first_name]
		end

		if society_params[:p_last_name]
			self.p_last_name = society_params[:p_last_name]
		end

		if society_params[:p_phone]
			self.p_phone = society_params[:p_phone]
		end 

		if society_params[:p_email]
			self.p_email = society_params[:p_email]
		end

		if society_params[:announcement]
			self.announcement = society_params[:announcement]
		end
	...
	def permited_params(params)
		params.require(:society).permit(:name, :desc, :regi_num, :p_first_name, :p_last_name, :p_phone, :p_email, :state, :announcement
			)
	end
end